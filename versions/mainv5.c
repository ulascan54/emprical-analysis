...
void *performSorts(void *arguments)
{
    ...
    snprintf(logFilename, sizeof(logFilename), "%s_times.txt", args->fileName); // Create filename for log
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        fprintf(logFile, "Run %d: %f seconds\n", i + 1, times[i]); // Log each run's time to file
    }
    double avgTime = averageTime(times, 5);
    fprintf(logFile, "Average time (excluding first run): %f seconds\n", avgTime); // Log average time
    fclose(logFile);

    saveArrayToFile(array, args->size, args->fileName); // Save the sorted array to a file
    free(array);
    return NULL;
}

// Wrapper functions and sorting algorithms need to be defined here.
